...
export interface ReviewBoardClient {
  listReviewRequests(status: "pending" | "submitted" | "all", fromUser?: string): Promise<ReviewBoardRequest[]>;
  ...
  getFileDiffData(requestId: number, diffSetId: number, fileDiffId: number): Promise<ReviewBoardDiffData>;
  ...
  addDiffComment(requestId: number, reviewId: number, fileDiffId: number, firstLine: number, numLines: number, text: string): Promise<void>;
   ...
   */
  postReview(requestId: number, summary: string, inlineComments?: Array<{
    fileDiffId: number;
    line: number;
    text: string;
    numLines?: number;
...
export async function rbRequest<T>(
  ...
  const authHeader = username 
    ...
    : (token.toLowerCase().startsWith("token ") ? token : `token ${token}`);
  ...
  if (data && typeof data === "object" && data.stat === "fail") {
...
export async function getCurrentUser(
  ...
  const userResponse = await rbRequest<{ user: { id: number; username: string; fullname: string } }>(
    baseUrl,
    token,
    userHref,
    {},
...
export function createReviewBoardClient(baseUrl: string, token: string, username?: string): ReviewBoardClient {
  const client: ReviewBoardClient = {
    async listReviewRequests(status, fromUser) {
      ...
      const response = await rbRequest<{ review_requests: ReviewBoardRequest[] }>(baseUrl, token, endpoint, {}, username);
      return response.review_requests || [];
    },

    async getReviewRequest(requestId) {
      const response = await rbRequest<{ review_request: ReviewBoardRequest }>(baseUrl, token, `/api/review-requests/${requestId}/?expand=submitter`, {}, username);
      return response.review_request;
    },

    async getLatestDiffSet(requestId) {
      const response = await rbRequest<{ diffs: ReviewBoardDiffSet[] }>(baseUrl, token, `/api/review-requests/${requestId}/diffs/`, {}, username);
      const diffs = response.diffs || [];
      return diffs.length > 0 ? diffs[diffs.length - 1] : null;
    },

    async getFileDiffs(requestId, diffSetId) {
      const response = await rbRequest<{ files: ReviewBoardFileDiff[] }>(baseUrl, token, `/api/review-requests/${requestId}/diffs/${diffSetId}/files/`, {}, username);
      return response.files || [];
    },

    async getFileDiffData(requestId, diffSetId, fileDiffId) {
      const response = await rbRequest<{ diff_data: ReviewBoardDiffData }>(baseUrl, token, `/api/review-requests/${requestId}/diffs/${diffSetId}/files/${fileDiffId}/diff-data/`, {}, username);
    ...
    async createReview(requestId, bodyTop) {
      const response = await rbRequest<{ review: ReviewBoardReview }>(baseUrl, token, `/api/review-requests/${requestId}/reviews/`, {
        method: "POST",
    ...
    async addDiffComment(requestId, reviewId, fileDiffId, firstLine, numLines, text) {
      await rbRequest(baseUrl, token, `/api/review-requests/${requestId}/reviews/${reviewId}/diff-comments/`, {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
    ...
    async publishReview(requestId, reviewId) {
      await rbRequest(baseUrl, token, `/api/review-requests/${requestId}/reviews/${reviewId}/`, {
        method: "PUT",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
    ...
    async postReview(requestId, summary, inlineComments = []) {
      ...
      for (const comment of inlineComments) {
        await this.addDiffComment(requestId, review.id, comment.fileDiffId, comment.line, comment.numLines || 1, comment.text);
      }
